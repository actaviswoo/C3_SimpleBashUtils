...
typedef struct flags {
  ...
  int n;  // Предваряет каждую строку вывода номером строки из файла ввода. | печать number
  int h;  // Выводит совпадающие строки, не предваряя их именами файлов. | печать line
  int s;  // Подавляет сообщения об ошибках о несуществующих или нечитаемых файлах. | 141
  int f;  // Получает регулярные выражения из файла. | печать line
  int o;  // Печатает только совпадающие (непустые) части совпавшей строки. | печать finded в line
} typeflags;
...
int efChecker(char ** pattern, char ** txt, int * start_read); // добавление pattern в отсутсвие флагов -e -f
void s21_grep(typeflags * flags, char * txtname, char ** pattern, int realcountertxt);
void freeArray(char ** pattern, char ** txt);
int finder(char * line, char ** pattern, char ** finded, typeflags * flags); // от найденного index и включенного флага -v зависит печать line
